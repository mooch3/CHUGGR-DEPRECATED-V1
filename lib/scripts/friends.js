...
var findFriends = document.getElementById('findFriends');
var currentUser = friendForm.me.value;
var usersRef = firestore.collection('users').where('uid', 'not-in', [currentUser]);
var friendRef = firestore.collection('users').doc(currentUser).collection('friends');
var currentRef = firestore.collection('users').doc(currentUser);
var fieldValue = firebase.firestore.FieldValue;
var potentialFriends = [];
var friendObjects = []; // create a protential friends array

usersRef.onSnapshot(function (allUsers) {
  ...
  console.log(friendObjects); // auto complete all friend possibilities
...
function addFriendFunc(friendObjects) {
  addFriend.addEventListener("click", function (e) {
    var regExpCheck = /\(([^)]+)\)/; // get the friend's username value from findFriends input

    var selectedFriend = regExpCheck.exec(findFriends.value);
    console.log(selectedFriend[1]);
    var filteredFriend = friendObjects.filter(function (object) {
      return object.userName == selectedFriend[1];
    });
    console.log(filteredFriend);

    if (filteredFriend[0].userName == selectedFriend[1]) {
      friendRef.doc(filteredFriend[0].uid).get().then(function (doc) {
